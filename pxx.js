...
20210410 tom
软件：拼夕夕
看广告免费拼商品，拼不到可获得一元红包，每天得到5元，满20提现。
新出的毛一般都能提现，但不确定能到不到账，看脸！！
进入软件后任意点击一款商品 → 看广告拼团 即可获取ck
需要支付宝授权认证，不刷脸，不需要支付。
此脚本只是拿来练手，提现不到账勿喷！
此脚本只是拿来练手，提现不到账勿喷！
此脚本只是拿来练手，提现不到账勿喷！
...
const $ = new Env('拼夕夕');
let pxxurl = $.getdata('pxxurl')
let pxxhd = $.getdata('pxxhd')
...
await $.wait(15000);
  ...
  .finally(() => $.done())
function pxxck() {
   if ($request.url.indexOf("MianFeiPT") > -1) {
    $.setdata($request.url,'pxxurl')
    $.log(pxxurl)
    $.setdata(JSON.stringify($request.headers),'pxxhd')
$.log(pxxhd)
    $.setdata($request.body,'pxxbody')
$.log(pxxbody)
   $.msg($.name,"","拼夕夕headers成功！")
    }
  }
function pxxkgg(timeout = 0) {
  return new Promise((resolve) => {
let url = {
        url : 'http://apipx.jc8s.com:4022/Index/MianFeiPT',
        headers : JSON.parse(pxxhd),
        body : pxxbody,}
